    ...
    while let Some(pos) = queue.pop_front() {
        ...
        for (new_pos, dir) in &adjacent_positions(pos) {
            if !discovered.contains(new_pos) {
                ...
                let tile = map.get(new_pos);
                if tile == Some(&Floor) || tile == Some(&Oxygen) {
...
fn run_program(initial_memory: &HashMap<i64, i64>) -> (HashMap<(i32, i32), Tile>, (i32, i32)) {
    ...
    let mut dir_back = None;

    let mut map = HashMap::new();
    map.insert((0, 0), Floor);
    let mut oxygen_position = None;
    let mut backtracking_stack = Vec::new();
    ...
    loop {
        ...
        match op {
            ...
            3 => {
                let mem_loc = memory[&(i+1)];
                let mut dead_end = true;
                for (adj, dir) in &adjacent_positions(robot_position) {
                    if map.get(adj) == None {
                        dead_end = false;
                        next_position = *adj;
                        dir_back = Some(dir.reverse());
                        set_data!(mode1, mem_loc, *dir as i64);
                        break;
                    }
                }

                if dead_end {
                    if let Some((pos, dir)) = backtracking_stack.pop() {
                        next_position = pos;
                        dir_back = None;
                        set_data!(mode1, mem_loc, dir as i64);
                    } else {
            ...
            4 => {
                match get_data!(mode1, memory[&(i+1)]) {
                    ...
                    1 => {
                        ...
                        if let Some(dir) = dir_back {
                            backtracking_stack.push((robot_position, dir));
                        }
