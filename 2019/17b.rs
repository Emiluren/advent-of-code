...
fn run_program(initial_memory: &HashMap<i64, i64>, mut input: impl Iterator<Item=char>) -> i64 {
    ...
    loop {
        ...
        match op {
            ...
            3 => {
                let pos = memory[&(i+1)];
                let input_value = input.next().unwrap() as i64;
                set_data!(mode1, pos, input_value);
                i += 2;
            }
            4 => {
                let output_value: i64 = get_data!(mode1, memory[&(i+1)]);
                if let Ok(next_u8) = TryInto::<u8>::try_into(output_value) {
                    print!("{}", next_u8 as char);
                } else {
                    return output_value;
                }
                i += 2;
    ...
    panic!("Did not get output value")
}
